...
RU: dict[str, Any] = {
    ...
    "tz_manual_prompt": "Отправь часовой пояс в формате IANA, например `Europe/London` или `America/New_York`. Короткие алиасы `EST`/`ET` тоже поддерживаются.",
    "tz_success": "✅ Часовой пояс: `{tz}`",
    "tz_invalid": "❌ Неизвестный часовой пояс. Используй формат IANA: `Europe/London` / `America/New_York`, либо алиас `EST`/`ET`.",
